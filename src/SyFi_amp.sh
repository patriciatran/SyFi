...
## CHECK: Log file
if [[ -f ${OUTPUT_FOLDER}/01-Logs/amplicon/log_${DATE}.txt ]]; then
	# Remove old log file
	rm -rf ${OUTPUT_FOLDER}/01-Logs/amplicon/log_${DATE}.txt
...
function ctrl_c() {
	...
	printf "\n${red}Execution halted by user.${normal}\n" >> ${OUTPUT_FOLDER}/01-Logs/amplicon/log_${DATE}.txt
	exit
}
...
function amplicon() {
	...
	if [ $mode == "unique" ];then
	 ...
	 QZAPATH=${OUTPUT_FOLDER}/71-Amplicon/${subf}
	 OUTPATH=${OUTPUT_FOLDER}/71-Amplicon/${subf}
	 ...
	 do
		...
		qiime tools import \
			 ...
			 --type 'FeatureData[Sequence]' &>> ${OUTPUT_FOLDER}/01-Logs/amplicon/log_${DATE}.txt
		qiime feature-classifier extract-reads \
			 ...
			 --o-reads $QZAPATH/${FNAME}_amp.qza &>> ${OUTPUT_FOLDER}/01-Logs/amplicon/log_${DATE}.txt
		qiime tools export \
			 --input-path $QZAPATH/${FNAME}_amp.qza \
			 --output-path $OUTPATH/${FNAME}_amp.fna &>> ${OUTPUT_FOLDER}/01-Logs/amplicon/log_${DATE}.txt
	...
	for i in $(ls ${OUTPUT_FOLDER}/71-Amplicon/${subf} | sed 's/.fasta//g'); do
		if [[ ! "${keep[*]}" =~ "${i}" && ${mode} == "multiple" ]]; then
			rm -f ${OUTPUT_FOLDER}/71-Amplicon/${subf}/${i}.fasta
		fi
	done

 # Concatenate haplotypes (NF-1 to obtain "per_haplotype")
		cat ${OUTPUT_FOLDER}/60-Integration/${subf}/integration.tsv | awk '{print $1 "/" $(NF-1)}' | tail -n +2 > ${OUTPUT_FOLDER}/71-Amplicon/${subf}/tmp.tsv
		for h in $(cat ${OUTPUT_FOLDER}/71-Amplicon/${subf}/tmp.tsv); do
			seq=$(echo ${h} | cut -d "/" -f 1)
			num=$(echo ${h} | cut -d "/" -f 2)
			for n in $(seq ${num}); do
				# End when haplotype ratio > 1
				if [[ ${n} == ${num} && ${n} != 1 ]]; then
					grep -v "^>" ${OUTPUT_FOLDER}/71-Amplicon/${subf}/${seq}.fasta >> ${OUTPUT_FOLDER}/71-Amplicon/${subf}/${subf}_all_haplotypes.tmp.fasta
				# End when haplotype ratio == 1
				elif [[ ${n} == ${num} && ${n} == 1 ]]; then
					grep -v "^>" ${OUTPUT_FOLDER}/71-Amplicon/${subf}/${seq}.fasta >> ${OUTPUT_FOLDER}/71-Amplicon/${subf}/${subf}_all_haplotypes.tmp.fasta
				# Addition of haplotypes before ending
					...
					grep -v "^>" ${OUTPUT_FOLDER}/71-Amplicon/${subf}/${seq}.fasta >> ${OUTPUT_FOLDER}/71-Amplicon/${subf}/${subf}_all_haplotypes.tmp.fasta
					echo "NNNNNNNNNN" >> ${OUTPUT_FOLDER}/71-Amplicon/${subf}/${subf}_all_haplotypes.tmp.fasta
				fi
			done
		done
		printf ">${subf}_all_haplotypes\n" > ${OUTPUT_FOLDER}/71-Amplicon/${subf}/${subf}_all_haplotypes.fasta
		cat ${OUTPUT_FOLDER}/71-Amplicon/${subf}/${subf}_all_haplotypes.tmp.fasta | tr -d "\n" >> ${OUTPUT_FOLDER}/71-Amplicon/${subf}/${subf}_all_haplotypes.fasta
...
function CleanFiles() {
	...
	# Array
	keep=("./${OUTPUT_FOLDER}/01-Logs/amplicon/log_${DATE}.txt" "./${OUTPUT_FOLDER}/10-Blast/${SAMPLE}.tsv" "./${OUTPUT_FOLDER}/11-Sequences/${SAMPLE}/${SAMPLE}.fasta" "./${OUTPUT_FOLDER}/20-Alignment/${SAMPLE}/${SAMPLE}.fasta" "./${OUTPUT_FOLDER}/20-Alignment/${SAMPLE}/${SAMPLE}_R1.fastq.gz" "./${OUTPUT_FOLDER}/20-Alignment/${SAMPLE}/${SAMPLE}_R2.fastq.gz" "./${OUTPUT_FOLDER}/30-VariantCalling/${SAMPLE}/variants/${SAMPLE}.vcf.gz" "./${OUTPUT_FOLDER}/40-Phasing/${SAMPLE}/${SAMPLE}_phased.vcf.gz" "./${OUTPUT_FOLDER}/50-haplotypes/${SAMPLE}/clean_${SAMPLE}_haplotypes.fasta" "./${OUTPUT_FOLDER}/60-Integration/${SAMPLE}/abundance.tsv" "./${OUTPUT_FOLDER}/60-Integration/${SAMPLE}/copy_number.tsv" "./${OUTPUT_FOLDER}/60-Integration/${SAMPLE}/integration.tsv" "./${OUTPUT_FOLDER}/70-Fingerprints/${SAMPLE}/${SAMPLE}_all_haplotypes.fasta" "./${OUTPUT_FOLDER}/71-Amplicon/${SAMPLE}/${SAMPLE}_all_haplotypes.fasta")
	...
	if [ ${KEEPF} -eq 0 ]; then
		...
		rm -rf ${OUTPUT_FOLDER}/71-Amplicon/${SAMPLE}/*tmp*
		...
		chmod uog+rwx ${OUTPUT_FOLDER}/71-Amplicon/${SAMPLE}
		chmod uog+rwx ${OUTPUT_FOLDER}/71-Amplicon/${SAMPLE}/*
	fi
}
...
for subf in $(ls ${INPUT_FOLDER}); do
	...
	if [ ${VERBOSE} -ge 1 ]; then date "+start time: %H:%M:%S" | tee -a ${OUTPUT_FOLDER}/01-Logs/amplicon/log_${DATE}.txt; fi
	if [ ${VERBOSE} -ge 1 ]; then printf "${blue}Sample:${normal} $subf\n" | tee -a ${OUTPUT_FOLDER}/01-Logs/amplicon/log_${DATE}.txt; fi

	#Check whether there is only one haplotype or not
	if [[ -f ${OUTPUT_FOLDER}/30-VariantCalling/${subf}/variants/${subf}.vcf.gz && $(zcat ${OUTPUT_FOLDER}/30-VariantCalling/${subf}/variants/${subf}.vcf.gz | grep -v "#" | wc -l) == 0 && $(grep "^>" ${OUTPUT_FOLDER}/20-Alignment/${subf}/${subf}.fasta | wc -l) == 1 ]]; then
 ...
 if [ -f ${OUTPUT_FOLDER}/71-Amplicon/${subf}/${subf}_all_haplotypes.fasta ]; then
	printf "${subf}\tAmplicon retrieved\n" >> progress_amp.txt
 fi

 CleanFiles ${subf} ${KEEPF}

 date "+end time: %d/%m/%Y - %H:%M:%S" | tee -a ${OUTPUT_FOLDER}/01-Logs/amplicon/log_${DATE}.txt
 echo ""
